...
// Time critical write: exactly like original
...
// ---------------------------------------------------------------------
// Base pattern (original animation) stored in flash, NEVER modified
// ---------------------------------------------------------------------
const byte baseAnimation[N_FRAMES][N_DIVS] PROGMEM =
...
// ---------------------------------------------------------------------
// Working table used in the time-critical loop
// ---------------------------------------------------------------------
static byte animation[N_FRAMES][N_DIVS];

// Relative phase offset of hemisphere 2 (A2/A3) relative to hemisphere 1 (A0/A1)
static byte phaseOffset = 0;

// Recompute animation[][] based on baseAnimation[][] and phaseOffset.
// Hemisphere 1 (A0/A1) uses baseAnimation[frame]
// Hemisphere 2 (A2/A3) uses baseAnimation[(frame + phaseOffset) % N_FRAMES]
// This runs OUTSIDE the timing-critical loop and may cause a brief glitch.
void updateAnimationWithOffset()
{
  for (byte f = 0; f < N_FRAMES; ++f) {
    byte f2 = f + phaseOffset;
    if (f2 >= N_FRAMES) f2 -= N_FRAMES;
    for (byte d = 0; d < N_DIVS; ++d) {
      byte base0 = pgm_read_byte(&baseAnimation[f][d]);
      byte base1 = pgm_read_byte(&baseAnimation[f2][d]);
      // lower 2 bits (A0, A1) from hemisphere 1
      // upper 2 bits (A2, A3) from hemisphere 2
      animation[f][d] = (base0 & 0b00000011) | (base1 & 0b00001100);
    }
  }
}
...
void setup()
{
  ...
  // initial animation: phaseOffset = 0 => exactly original pattern
  updateAnimationWithOffset();
  ...
  LOOP:
    ...
    // TIME-CRITICAL SECTION: structure identical to original
    ...
    OUTPUT_WAVE(emittingPointer, 2); buttonPressed[0] = buttonsPort & 0b00000100; WAIT_MID(); // D2
    OUTPUT_WAVE(emittingPointer, 3); buttonPressed[1] = buttonsPort & 0b00001000; WAIT_MID(); // D3
    OUTPUT_WAVE(emittingPointer, 4); buttonPressed[2] = buttonsPort & 0b00010000; WAIT_MID(); // D4
    OUTPUT_WAVE(emittingPointer, 5); buttonPressed[3] = buttonsPort & 0b00100000; WAIT_MID(); // D5 (relative phase)
    OUTPUT_WAVE(emittingPointer, 6); buttonPressed[4] = buttonsPort & 0b01000000; WAIT_MID(); // D6
    OUTPUT_WAVE(emittingPointer, 7); buttonPressed[5] = buttonsPort & 0b10000000; WAIT_MID(); // D7
    ...
    if (anyButtonPressed) {
      ...
      if (buttonCounter > BUTTON_SENS) {
        ...
        // D2: step frame backward (both hemispheres together)
        if (!buttonPressed[0]) {
          if (frame < STEP_SIZE) { 
            frame = N_FRAMES - 1;
          } else {
        ...
        // D3: step frame forward (both hemispheres together)
        ...
        // D4: reset frame (both hemispheres)
        else if (!buttonPressed[2]) { 
          frame = 0;
        }
        // D5: increase relative phase of hemisphere 2 by one frame (15 degrees)
        else if (!buttonPressed[3]) {
          phaseOffset++;
          if (phaseOffset >= N_FRAMES) phaseOffset = 0;
          // This recalculation is allowed to glitch timing briefly.
          updateAnimationWithOffset();
        }
